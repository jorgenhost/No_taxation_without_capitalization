...
forvalues i=2000/2008 {
	...
	gen year`i'_delta_tax_service = year`i'*delta_tax_service
}
...
***************************************
*** W trend diff? 2004 as base year****
*** Incl. changes to income tax *******
***************************************
eststo clear

forvalues i = 10(10)90 {
	preserve
	eststo quant_est_`i': qui mmqreg  ln_real_price  year2000_delta_tax_eff year2001_delta_tax_eff year2002_delta_tax_eff year2003_delta_tax_eff year2005_delta_tax_eff year2006_delta_tax_eff year2007_delta_tax_eff year2008_delta_tax_eff year2005_delta_tax_service year2006_delta_tax_service year2007_delta_tax_service year2008_delta_tax_service ln_prop_value, absorb(year kommune_old_id) quantile(`i') cluster(index)
	...
	replace q = q/100
	order q, first
	save models\trend_diff\quant_est_`i'_trend_diff_04_robust, replace
	restore
}

esttab using "tabs\stata_main_qreg_robust_raw.tex", style(tex) replace star(* 0.10 ** 0.05 *** 0.01) se b(%5.3f) substitute({l} {p{\linewidth}})
